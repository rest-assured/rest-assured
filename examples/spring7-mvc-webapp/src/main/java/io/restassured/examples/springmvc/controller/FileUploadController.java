...
public class FileUploadController {
    ...
    @RequestMapping(value = "/fileUpload", method = { POST, PUT, PATCH } , consumes = MULTIPART_FORM_DATA_VALUE, produces = APPLICATION_JSON_VALUE)
    public @ResponseBody String fileUpload(@RequestParam("file") MultipartFile file) {
        return "{ \"size\" : "+file.getSize()+", \"name\" : \""+file.getName()+"\" }";
    }

    @RequestMapping(value = "/fileUpload2", method = { POST, PUT, PATCH }, consumes = MULTIPART_FORM_DATA_VALUE, produces = APPLICATION_JSON_VALUE)
    ...
    @RequestMapping(value = "/multiFileUpload", method = { POST, PUT, PATCH }, consumes = MULTIPART_FORM_DATA_VALUE, produces = APPLICATION_JSON_VALUE)
    ...
    @RequestMapping(value = "/fileUploadWithParam", method = { POST, PUT, PATCH }, consumes = MULTIPART_FORM_DATA_VALUE, produces = APPLICATION_JSON_VALUE)
    ...
    @RequestMapping(value = "/fileUploadWithControlNameEqualToSomething", method = { POST, PUT, PATCH }, consumes = MULTIPART_FORM_DATA_VALUE, produces = APPLICATION_JSON_VALUE)
    public @ResponseBody String fileUploadWithControlNameEqualToSomething(@RequestParam(value = "something") MultipartFile file) {
        return "{ \"size\" : "+file.getSize()+", \"name\" : \""+file.getName()+"\", \"originalName\" : \""+file.getOriginalFilename() + "\", \"mimeType\" : \""+file.getContentType()+"\" }";
    }

    @RequestMapping(value = "/textAndReturnHeader", method = { POST, PUT, PATCH }, consumes = "multipart/mixed", produces = APPLICATION_JSON_VALUE)
